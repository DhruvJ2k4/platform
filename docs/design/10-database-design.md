# 10 · Database Design
**Summary:** DuckDB over Parquet; ~14 core tables; partitioning by year; sorting in place of indexes. **Purpose:** physical schema. **Scope:** curated + operational tables (raw is files + registry). **Assumptions:** ADR-003/016. **Risks:** schema migrations (append-and-view policy below). **Open questions:** none blocking. **Future extensions:** BSE via `exchange` columns already present.
...
## Core DDL (representative; authoritative copies live in schemas/)
```sql
...
CREATE TABLE prices_adj (isin TEXT, d DATE, exchange TEXT, series TEXT,
  o DECIMAL, h DECIMAL, l DECIMAL, c DECIMAL, close_unadj DECIMAL,
  volume BIGINT, traded_value DECIMAL, adj_factor DOUBLE,
  band_hit TEXT,                                                                -- upper|lower|null
  PRIMARY KEY (isin, d));
CREATE TABLE corporate_actions (isin TEXT, ex_date DATE, kind TEXT,             -- split|bonus|dividend|demerger|rights|buyback
  ratio_num INT, ratio_den INT, cash_amount DECIMAL, status TEXT,               -- auto|needs_review|resolved
  source_ref TEXT, available_at TIMESTAMP);
CREATE TABLE universe_membership (isin TEXT, d DATE, investable BOOL,
  mdtv DECIMAL, amihud DOUBLE, zero_days_pct DOUBLE, surveillance TEXT,
...
CREATE TABLE fundamentals_pit (isin TEXT, period_end DATE, statement TEXT,
  item TEXT, value DECIMAL, filed_at TIMESTAMP, available_at TIMESTAMP,
...
CREATE TABLE index_tri (index_name TEXT, d DATE, tri_value DECIMAL);
-- operational (append-only, in backup set):
CREATE TABLE proposal (proposal_id TEXT PRIMARY KEY, book_id TEXT, asof DATE,
...
CREATE TABLE "order" (order_id TEXT PRIMARY KEY, proposal_id TEXT, isin TEXT,
  side TEXT, qty INT, limit_hint DECIMAL, reasons JSON);
CREATE TABLE fill (fill_id TEXT PRIMARY KEY, order_id TEXT, d DATE, qty INT,
  price DECIMAL, charges JSON, source TEXT);                                    -- source: contract_note|manual
CREATE TABLE lot (lot_id TEXT PRIMARY KEY, book_id TEXT, isin TEXT,
  open_d DATE, open_price DECIMAL, qty_open INT, qty_remaining INT);            -- FIFO consumption recorded in fill links
...
changes require rebuild (which we can always do — ADR-016).
